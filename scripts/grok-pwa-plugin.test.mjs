...
// The injector's cwd defaults to process.cwd(), which is this repo: Gridiron Lab's
// own src/lib/og/site.json title and public/og.jpg would override the title and
// card fallbacks under test. Calls run in an empty workspace unless a test names one.
const NO_IDENTITY = mkdtempSync(join(tmpdir(), "grok-og-no-identity-"));
const injectHead = (html, ctx = {}) => injectGrokPwaHead(html, { cwd: NO_IDENTITY, ...ctx });
const headInjector = (ctx = {}) => createHeadInjector({ cwd: NO_IDENTITY, ...ctx });

test("injects before </head>", () => {
  const out = injectHead("<html><head><title>x</title></head><body></body></html>");
...
test("injects the extensions script without a project id", () => {
  const out = injectHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "",
  });
...
test("does not duplicate grok:app_id", () => {
  ...
  const once = injectHead("<html><head></head></html>", ctx);
  const twice = injectHead(once, ctx);
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectHead(html, { appName: "Wild Race" });
...
test("does not duplicate twitter:card or og:title", () => {
  const once = injectHead("<html><head><title>Hello World</title></head></html>");
  const twice = injectHead(once);
  assert.equal(once, twice);
...
test("does not emit x:game:image without a public host or banner", () => {
  const noHost = injectHead("<html><head></head></html>", {
    site: { banner: "/x-banner.jpg" },
  });
  assert.doesNotMatch(noHost, /x:game:image/);
  const noBanner = injectHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
...
test("published VITE_PUBLIC_HOSTNAME wins over request Host for og:image", () => {
  ...
  try {
    const vercelHost = injectHead("<html><head><title>RACK</title></head></html>", {
    ...
    const otherPublicHost = injectHead("<html><head><title>RACK</title></head></html>", {
...
test("vercel Host without a public hostname emits no og:image", () => {
  ...
  try {
    const out = injectHead("<html><head><title>RACK</title></head></html>", {
      host: "01a020b6-803a-71a2-bb47-e2bec57eb9a2-662k8x1l1-xai-org.vercel.app",
      site: { title: "RACK", card: "custom" },
    });
    assert.doesNotMatch(out, /property="og:image"/);
...
test("emits og:image for a public host and prefers a custom card", () => {
  const placeholder = injectHead("<html><head></head></html>", {
    appName: "Wild Race",
    host: "wild-race.grok.me",
    site: { title: "Wild Race" },
  });
  ...
  const custom = injectHead("<html><head></head></html>", {
    appName: "Wild Race",
    host: "wild-race.grok.me",
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = injectHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
  ...
  const invalid = injectHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
...
test("injects into documents with no head element", () => {
  const out = injectHead("<html><body>hi</body></html>", { appName: "Solo" });
...
test("is idempotent", () => {
  const once = injectHead("<html><head></head></html>");
  const twice = injectHead(once);
  assert.equal(once, twice);
});

test("uses the app name in the injected title tag", () => {
  const out = injectHead("<html><head></head></html>", { appName: "Wild Race" });
